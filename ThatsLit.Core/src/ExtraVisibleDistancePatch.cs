    ...
    public class ExtraVisibleDistancePatch : ModulePatch
    {
        ...
        public static bool PatchPrefix(EnemyInfo __instance)
        {
            ...
            if (thermalActive)
            {
                ...
                if (compensation > 0) __instance.Owner.LookSensor.VisibleDist += UnityEngine.Random.Range(0.5f, 1f) * compensation * ThatsLitPlugin.ExtraVisionDistanceScale.Value;
            ...
            else if (nvgActive && frame0.ambienceScore < 0) // Base + Sun/Moon < 0
            {
                ...
                if (compensation > 0)
                {
                    ...
                    __instance.Owner.LookSensor.VisibleDist += UnityEngine.Random.Range(0.5f, 1f) * compensation * ThatsLitPlugin.ExtraVisionDistanceScale.Value;
                }
                if (isNearest && player.DebugInfo != null)
